...
public class BrowserRestController {
    ...
    public void getUrl(@PathVariable final String url) {
        log.info("url: {}", url);
        //如果没有配置环境变量，需要调用
        //第一个参数固定写法，第二个参数是chromedriver的安装位置
        System.setProperty("webdriver.chrome.driver", "C:\\ideaPorject\\ace\\doc\\chrome\\windows\\chromedriver_105.exe");
        try {
            //设置不弹出浏览器
