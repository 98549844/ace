...
//BeanNameGenerator接口实现类UML关系图
/**
                                BeanNameGenerator
                                    ^      ^
                                    |       \
            AnnotationBeanNameGenerator     BefaultBeanNameGenerator
                            ^
                            |
...
public class DefaultBeanNameGenerator implements BeanNameGenerator {
    ...
    public static String uniqueBeanName(String beanName, BeanDefinitionRegistry registry) {
        ...
        /*AnnotationBeanNameGenerator是BeanNameGenerator接口的实现类,
        用来生成被@Component注解或者其它@Component派生注解标注的bean;
        示例: 被@Component注解标注的@Repository注解.

        如果注释的值没有指示bean名称, 那么将基于类的短名称(第一个字母消息)
        构建一个适当的名称. 例如: com.xyz.FooServiceImpl类的bean名称是fooServiceImpl.*/
...
/*
    ConfigurationClassPostProcessor#setBeanNameGenerator方法设置自定义bean name生成器,
    这个setter通常只适用于将ConfigurationClassPostProcessor后处理器配置为XML中的独立bean定义,
    例如: 不使用专用的AnnotatiionConfig*应用程序上下文或者context:annotation-config标签,
    针对应用程序上下文指定的任何bean名称生成器都将优先于此处的设置(此方法只适合Spring中通过xml配置的形式, springboot可以忽略)
*/
...
/*    到这里会有一个疑问内部自定义bean name生成策略
    bean org.springframework.context.annotation.internalConfigurationBeanNameGenerator
        (AnnotationConfigUtils.CONFIGURATION_BEAN_NAME_GENERATOR)
        是这样注入到IOC容器之中的. 那我们看下启动类SpringApplication#run方法,
        方法中内嵌prepareContext方法, prepareContext方法内嵌postProcessApplicationContext方法:*/
    ...
    protected void postProcessApplicationContext(ConfigurableApplicationContext context) {
        ...
        /*此段代码我们看到了如何将bean name自定义生成策略注入到IOC容器之中,
        那么beanNameGenerator生成策略是这样获取. beanNameGenerator是SpringApplication类的一个属性,
        提供了setter方法*/
